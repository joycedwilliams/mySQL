...
-- the name of Harpo's second cousin's husband's yoga teacher. Write a query to fix the record.
  	UPDATE actor
    actor_name SET actor_name = "HARPO WILLIAMS" WHERE actor_name = "GROUCHO WILLIAMS";
...
-- In a single query, if the first name of the actor is currently `HARPO`, change it to `GROUCHO`. Otherwise, change the first name to `MUCHO GROUCHO`, as that is exactly what the actor will be with the grievous error. BE CAREFUL NOT TO CHANGE THE FIRST NAME OF EVERY ACTOR TO `MUCHO GROUCHO`, HOWEVER! (Hint: update the record using a unique identifier.)
	UPDATE actor
	actor_name SET actor_name = "GROUCHO WILLIAMS" WHERE actor_name = "HARPO WILLIAMS";
...
-- Use joins to retrieve this information.
	SELECT city.last_update
    FROM city;
    
    SELECT country.last_update, country.country
    FROM country

	-- SELECT customer.customer_id, customer.first_name, customer.last_name, customer.email
    
    -- FROM country
	INNER JOIN customer ON customer.last_update = country.last_update
...
-- * 7f. Write a query to display how much business, in dollars, each store brought in.
    ...
    FROM store
    INNER JOIN store ON store.last_update = payment.last_update;
...
-- (**Hint**: you may need to use the following tables: category, film_category, inventory, payment, and rental.)
  	SELECT film_category.category_id, film_film_id
    FROM film_category;
    
    SELECT category.category_id, category.film_id
    FROM category;
...
-- Use the solution from the problem above to create a view. If you haven't solved 7h, you can substitute another query to create a view.
    ...
    CREATE VIEW sakila.v AS SELECT * FROM film;
    
-- * 8b. How would you display the view that you created in 8a?
-- A View would be selected like a Table is selected.

-- * 8c. You find that you no longer need the view `top_five_genres`. Write a query to delete it.
	REPLACE VIEW "top_five_genres AS
	SELECT name
	FROM sakila
	WHERE View = "top_five_genres";_